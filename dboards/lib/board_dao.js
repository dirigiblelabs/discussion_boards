...
var setTags = function(id, tags, createOnDemand){
	...
	if(boardTags){
		for(var i=0; i < boardTags.length; i++){
			this.$log.info('Removing {} entity relation between {}[{}] entity and ANN_TAG[{}]', tagRefsDAO.orm.table, this.orm.getPrimaryKey().column, id, boardTags[i].id);
			...
			if(updatedRecordsCount > 0)
		    	this.$log.info('{} entity relation between {}[{}] entity and ANN_TAG[{}] removed', tagRefsDAO.orm.table, this.orm.getPrimaryKey().column, id, boardTags[i].id);
		}
